...
public class LendServiceImpl extends ServiceImpl<LendMapper, Lend> implements LendService {
    ...
    public void makeLoan(Long id) {
        Lend lend=baseMapper.selectById(id);
        Assert.notNull(lend, ResponseEnum.NOT_LEND_ERROR);
        ...
        try{
            ...
            //发送同步远程调用
            result = RequestHelper.sendRequest(paramMap, HfbConst.MAKE_LOAD_URL);
            log.info("放款结果：" + result.toJSONString());
        }catch (Exception e){
            log.error("汇付宝连接异常", e);
            throw new BusinessException("汇付宝连接异常");
        }
...
//        lend.setStatus(LendStatusEnum.CHECK.getStatus());
//        baseMapper.update();
    }
    ...
    public List<LendItemReturn> returnInvest(Long lendItemId, Map<Integer, Long> lendReturnMap, Lend lend) {
        ...
        for (Map.Entry<Integer, BigDecimal> entry : mapInterest.entrySet()) {
            ...
            BigDecimal principal = Objects.requireNonNullElse(lendItemReturn.getPrincipal(), BigDecimal.ZERO);
            BigDecimal interest = Objects.requireNonNullElse(lendItemReturn.getInterest(), BigDecimal.ZERO);
